...
from .plugin import ActionPlugin
import math
import csv
from . import util
import json
...
class VaccinePlugin(ActionPlugin):
    ...
    def change_vaccine_level(self, pop_template, values:dict, cycle_step:int, sim_step:int):
        ...
        for n in target_node.contained_blobs:
            ...
            if n.get_traceable_characteristic('vaccine_level') == current_level:    
                ...
                _blob = n.split_and_change_blob_traceable_characteristic('vaccine_level', current_level + 1, values['quantity'])
                if _blob != n:
