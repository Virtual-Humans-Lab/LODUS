...
from util.math import (
    distance2D,
    distribute_ints_from_weights_with_limit,
)
...
class GatherPopulationPlugin(ActionPlugin):
    ...
    def load_plugin(self, simulation: LodusSimulation):
        ...
        self.percentage_per_search:float = self.config.get("percentage_per_search", 0.05)
    ...
    def get_nodes_distance(self, node1:EnvNode, node2:EnvNode)->float:
        ...
        self.distances_map[n1_name+n2_name] = distance2D(node1.long_lat, node2.long_lat)
        return self.distances_map[n1_name+n2_name]
    ...
    def gather_population(self, pop_template, values:dict, cycle_step:int, sim_step:int):
        ...
        int_weights = distribute_ints_from_weights_with_limit(quantity,
        ...
        self.add_execution_time(
            "gather_population", time.perf_counter() - start_time
        )
