...
def create_levy_distribution_figure(location: float, scale: float, size: int,
    ...
    fig_path = dir_path / f"levy-distribution-L{location}-S{scale}-Step{bin_step}-Q{size}.png"
    ...
    fig_path = dir_path / f"levy-distribution-L{location}-S{scale}-Step{bin_step}-Q{size}cumulative.png"
    plt.ylim(0, size)
    plt.savefig(fig_path, dpi=400)
    plt.clf()
...
if __name__ == '__main__':
    ...
    create_levy_distribution_figure(location=args['l'], scale=args['s'], size=args['q'],
